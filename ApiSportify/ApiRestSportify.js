...
app.post("/users/register", function(request, response) {
    ...
    connection.query(sql1, params1, function(err, res) {
        if (err) {
            ...
            let params = [request.body.name, request.body.lastName, request.body.password, 
                        request.body.rol, request.body.email, request.body.phone];
...
app.put("/users", function(request, response) {
    let params = [request.body.name, request.body.lastName, request.body.password, 
        request.body.rol, request.body.email, request.body.phone, request.body.user_id];
...
app.get("/teams/:user_id", function(request, response) {
    ...
    connection.query(sql, params, function(err, resultado) {
        if (err) {
            ...
            console.log("GET teams created by user");
            response.send(resultado);
        }
    });
});
...
app.get("/training/players/:user_id", function(request, response) {
    ...
    let sql = `SELECT training_team.team_id, training.*, team.name AS team FROM training AS training 
     INNER JOIN training_team AS training_team ON ( training.training_id = training_team.training_id ) 
     INNER JOIN team AS team ON ( training_team.team_id = team.team_id ) 
     INNER JOIN user_teams AS user_teams ON ( team.team_id = user_teams.team_id ) 
     INNER JOIN users AS users ON ( user_teams.user_id = users.user_id ) 
     WHERE users.user_id = ? ORDER BY training.date ASC`;
...
app.get("/match/player/:user_id", function(request, response) {
    ...
    let sql = `SELECT matches_teams.team_id,  matches_teams.match_id, team.name, matches.date, matches.rival, matches.location, matches.comments
     ...
     INNER JOIN matches_teams AS matches_teams ON (matches.match_id = matches_teams.match_id) 
     INNER JOIN team AS team ON (matches_teams.team_id = team.team_id) 
     INNER JOIN user_teams AS user_teams ON (team.team_id = user_teams.team_id) 
...
app.get("/tournament/player/:user_id", function(request, response) {
    let params = [request.params.user_id];
    let sql = `SELECT tournament.* FROM tournament  
